...
struct CartView: View {
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 0) {
            ...
            if(cartTotal > 0){
                let byCartInfos : BYCartInfos = self.getCartByInfos(shoppingCartProducts: items)
                AppDelegate.instance.beyableClient.sendPageview(page: EPageUrlTypeBeyable.CART, currentView: self,  attributes: BYCartAttributes(tags: ["totalPriceAmount":"\(cartTotal)"]), cartInfos: byCartInfos)
    ...
    func getCartByInfos(shoppingCartProducts : [Product]) -> BYCartInfos{
        var items = [BYCartItemInfos]()
        for cartProduit in shoppingCartProducts {
            
            items.append(BYCartItemInfos(productReference: cartProduit.uuid, productName: cartProduit.title, productUrl: "", productPrice: cartProduit.price, quantity: 1, thumbnailUrl: "", tags: [:]))
        }
